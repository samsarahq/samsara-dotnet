...
[TestFixture]
public class GetDriverTachographFilesTest : BaseMockServerTest
{
    ...
    public async Task MockServerTest()
    {
        ...
        Server
            .Given(
                WireMock
                    ...
                    .WithPath("/fleet/drivers/tachograph-files/history")
        ...
        var response = await Client.TachographEuOnly.GetDriverTachographFilesAsync(
            new GetDriverTachographFilesRequest { StartTime = "startTime", EndTime = "endTime" }
        );
        Assert.That(
            response,
            Is.EqualTo(JsonUtils.Deserialize<TachographDriverFilesResponse>(mockResponse))
                .UsingDefaults()
        );
    }
}
