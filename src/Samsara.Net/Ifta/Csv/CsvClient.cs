...
public partial class CsvClient
{
    private RawClient _client;

    internal CsvClient(RawClient client)
    {
        _client = client;
    }
    ...
    /// );
    /// </code></example>
    public async Task<IftaCreateIftaDetailJobResponseBody> CreateAsync(
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                return JsonUtils.Deserialize<IftaCreateIftaDetailJobResponseBody>(responseBody)!;
            }
    ...
    /// </code></example>
    public async Task<IftaGetIftaDetailJobResponseBody> GetAsync(
        string id,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                return JsonUtils.Deserialize<IftaGetIftaDetailJobResponseBody>(responseBody)!;
            }
            catch (JsonException e)
            {
                throw new SamsaraException("Failed to deserialize response", e);
