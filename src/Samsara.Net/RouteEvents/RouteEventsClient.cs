...
public partial class RouteEventsClient : IRouteEventsClient
{
    private RawClient _client;

    internal RouteEventsClient(RawClient client)
    {
        _client = client;
    }

    private async Task<
        WithRawResponse<RoutesGetRouteEventsStreamResponseBody>
    > GetRouteEventsStreamAsyncCore(
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<RoutesGetRouteEventsStreamResponseBody>(
                    responseBody
                )!;
                return new WithRawResponse<RoutesGetRouteEventsStreamResponseBody>()
                {
                    Data = responseData,
    ...
    /// </code></example>
    public WithRawResponseTask<RoutesGetRouteEventsStreamResponseBody> GetRouteEventsStreamAsync(
        GetRouteEventsStreamRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<RoutesGetRouteEventsStreamResponseBody>(
            GetRouteEventsStreamAsyncCore(request, options, cancellationToken)
        );
    }
}
