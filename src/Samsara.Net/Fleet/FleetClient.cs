...
public partial class FleetClient : IFleetClient
{
    ...
    private async Task<
        WithRawResponse<FleetLocationsGetFleetLocationsResponseBody>
    > GetFleetLocationsAsyncCore(
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            {
                var responseData =
                    JsonUtils.Deserialize<FleetLocationsGetFleetLocationsResponseBody>(
                        responseBody
                    )!;
                return new WithRawResponse<FleetLocationsGetFleetLocationsResponseBody>()
                {
                    Data = responseData,
    ...
    /// </code></example>
    public WithRawResponseTask<FleetLocationsGetFleetLocationsResponseBody> GetFleetLocationsAsync(
        GetFleetLocationsRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<FleetLocationsGetFleetLocationsResponseBody>(
            GetFleetLocationsAsyncCore(request, options, cancellationToken)
        );
    }
}
